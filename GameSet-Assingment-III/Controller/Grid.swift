...
struct Grid
{
    ...
    private mutating func recalculate() {
        ...
        case .aspectRatio:
            ...
            if cellSize.area > 0 {
                calculatedDimensions.columnCount = Int(frame.size.width / cellSize.width)
                if calculatedDimensions.columnCount > 9 { // ограничение количества столбцов
                    calculatedDimensions.columnCount = 9
                }
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
        ...
        origin.x += offset.dx
        origin.y = 0 //+= offset.dy // = 0 привязка к верхнему краю
